...
import org.corpus_tools.peppermodules.alignment.AlignmentExporter;
 ...
 * {@link AlignmentExporter} class. Feel free to adapt and enhance this test class
 ...
 * @author Martin Klotz
 */
public class AlignmentExporterTest extends PepperExporterTest {
	/**
	...
	@Before
	public void setUp() {
		setFixture(new AlignmentExporter());
	...
	public void test_DummyImplementation() {
		...
		getFixture().setCorpusDesc(new CorpusDesc().setCorpusPath(URI.createFileURI(getTempPath("AlignmentExporter").getAbsolutePath())));

		// starts the Pepper framework and the conversion process
		start();

		File superCorpus = new File(getTempPath("AlignmentExporter").getAbsolutePath() + "/rootCorpus");
